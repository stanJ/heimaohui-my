...
<html>
        ...
        <script type="text/javascript">
        	...
        	function displayContent(){
	        	...
	        	if(currentModel==null){  
    				...
    				/******************从页面中取出页面中的条件*******************************/
    				  					  logo=$("#search_logo").val();
					    					  ...
					    					  versionno=$("#search_versionno").val();
					    					  tel=$("#search_tel").val();
        			...
        			/******************从sessionStorge中取出页面中的条件*******************************/
        			  					  logo=sessionStorage.getItem(currentModel+".logo");
					   					  pics=sessionStorage.getItem(currentModel+".pics");
					   					  versionno=sessionStorage.getItem(currentModel+".versionno");
					   					  tel=sessionStorage.getItem(currentModel+".tel");
					   					  content=sessionStorage.getItem(currentModel+".content");
					     				/******************从sessionStorge中取出页面中的条件*******************************/
    				/******************将原来记住的值传重新赋值到页面上*******************************/
    				      				$("#search_logo").val(logo);
    				        				$("#search_pics").val(pics);
    				        				$("#search_versionno").val(versionno);
    				        				$("#search_tel").val(tel);
				...
				/************************拼装查询条件************************************/
					...
					_jsonFilter += "'search_EQ_logo':'"+logo+"',";
				}
			        					if(pics!=null && pics.length>0){
					_jsonFilter += "'search_EQ_pics':'"+pics+"',";
				}
			        					if(versionno!=null && versionno.length>0){
					_jsonFilter += "'search_EQ_versionno':'"+versionno+"',";
				}
			        					if(tel!=null && tel.length>0){
					_jsonFilter += "'search_EQ_tel':'"+tel+"',";
				}
				...
				_jsonFilter += "}";
				var url = "../cms/aboutus/findAboutusNEStatusPage";
				$.ajax({
					url:url,
					dataType:'json',
					type:'POST',
			...
			$(function(){
				...
				$("#updateSome").click(function(){
					...
					rememberRecord();
					var url = "aboutusEdit.jsp?id="+updateArray[0];
			    	window.location.href=url;
				});
				...
				$("#deleteSome").click(function() {
                    ...
                    if(confirm("你确定要删除选中项吗？")){
                    	var url="../cms/aboutus/operateAboutusByIds?modelIds=" + delArray.join(",");
                    	$.ajax({
							url: url,
							type: 'post',
							dataType: 'json',
				...
				$("#lockSome").click(function() {
                    ...
                    if(confirm("你确定要锁定选中项吗？")){
                    	var url="../cms/aboutus/operateAboutusByIds";
                    	$.ajax({
							url: url,
							type: 'post',
							data:{
    ...
    <body>
    <input type="hidden" id="currentModel" value="Aboutus" />
		<section>
			<div class="col-xs-12">
				<div class="box">
					<div class="span1">
						<span class="badge badge-info">XXXX   &gt; 关于我们管理</span>
					</div>
