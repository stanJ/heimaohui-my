...
<html>
        ...
        <script type="text/javascript">
        	...
        	function displayContent(){
	        	...
	        	if(currentModel==null){  
        			...
        			/******************从sessionStorge中取出页面中的条件*******************************/
        			  					  fileName=sessionStorage.getItem(currentModel+".fileName");
					   					  uptime=sessionStorage.getItem(currentModel+".uptime");
					   					  filePath=sessionStorage.getItem(currentModel+".filePath");
					     				/******************从sessionStorge中取出页面中的条件*******************************/
    				/******************将原来记住的值传重新赋值到页面上*******************************/
    				      				$("#search_fileName").val(fileName);
    				        				$("#search_uptime").val(uptime);
				...
				/************************拼装查询条件************************************/
					...
					_jsonFilter += "'search_EQ_fileName':'"+fileName+"',";
				}
			        					if(uptime!=null && uptime.length>0){
					_jsonFilter += "'search_EQ_uptime':'"+uptime+"',";
				}
				...
				_jsonFilter += "}";
				var url = "../cms/intro/findIntroNEStatusPage";
				$.ajax({
					url:url,
					dataType:'json',
					type:'POST',
			...
			$(function(){
				...
				$("#updateSome").click(function(){
					...
					rememberRecord();
					var url = "introEdit.jsp?id="+updateArray[0];
			    	window.location.href=url;
				});
				...
				$("#deleteSome").click(function() {
                    ...
                    if(confirm("你确定要删除选中项吗？")){
                    	var url="../cms/intro/operateIntroByIds?modelIds=" + delArray.join(",")+"&token="+encodeURIComponent(token);;
                    	$.ajax({
							url: url,
							type: 'post',
							dataType: 'json',
				...
				$("#lockSome").click(function() {
                    ...
                    if(confirm("你确定要锁定选中项吗？")){
                    	var url="../cms/intro/operateIntroByIds";
                    	$.ajax({
							url: url,
							type: 'post',
							data:{
    ...
    <body>
    <input type="hidden" id="currentModel" value="Intro" />
		<section>
			<div class="col-xs-12">
				<div class="box">
					<div class="span1">
						<span class="badge badge-info">XXXX   &gt; 公司简介管理</span>
					</div>
