...
<html>
        ...
        <script type="text/javascript">
        	...
        	function displayContent(){
	        	...
	        	if(currentModel==null){  
    				...
    				/******************从页面中取出页面中的条件*******************************/
    				  					  customerId=$("#search_customerId").val();
					    					  serial=$("#search_serial").val();
					    					  linker=$("#search_linker").val();
					    					  expertName=$("#search_expertName").val();
        			...
        			/******************从sessionStorge中取出页面中的条件*******************************/
        			  					  customerId=sessionStorage.getItem(currentModel+".customerId");
					   					  serial=sessionStorage.getItem(currentModel+".serial");
					   					  linker=sessionStorage.getItem(currentModel+".linker");
					   					  expertName=sessionStorage.getItem(currentModel+".expertName");
    				...
    				/******************将原来记住的值传重新赋值到页面上*******************************/
    				      				$("#search_customerId").val(customerId);
    				        				...
    				        				$("#search_linker").val(linker);
    				        				$("#search_expertName").val(expertName);
				...
				/************************拼装查询条件************************************/
					...
					_jsonFilter += "'search_EQ_customerId':'"+customerId+"',";
				}
			        					if(serial!=null && serial.length>0){
					_jsonFilter += "'search_EQ_serial':'"+serial+"',";
				}
			        					if(linker!=null && linker.length>0){
					_jsonFilter += "'search_EQ_linker':'"+linker+"',";
				}
			        					if(expertName!=null && expertName.length>0){
					_jsonFilter += "'search_EQ_expertName':'"+expertName+"',";
				}
				...
				_jsonFilter += "}";
				var url = "../cms/work/findWorkNEStatusPage";
				$.ajax({
					url:url,
					dataType:'json',
					type:'POST',
			...
			$(function(){
				...
				$("#updateSome").click(function(){
					...
					rememberRecord();
					var url = "workEdit.jsp?id="+updateArray[0];
			    	window.location.href=url;
				});
				...
				$("#deleteSome").click(function() {
                    ...
                    if(confirm("你确定要删除选中项吗？")){
                    	var url="../cms/work/operateWorkByIds?modelIds=" + delArray.join(",")+"&token="+encodeURIComponent(token);;
                    	$.ajax({
							url: url,
							type: 'post',
							dataType: 'json',
				...
				$("#lockSome").click(function() {
                    ...
                    if(confirm("你确定要锁定选中项吗？")){
                    	var url="../cms/work/operateWorkByIds";
                    	$.ajax({
							url: url,
							type: 'post',
							data:{
    ...
    <body>
    <input type="hidden" id="currentModel" value="Work" />
		<section>
			<div class="col-xs-12">
				<div class="box">
					<div class="span1">
						<span class="badge badge-info">XXXX   &gt; 合作记录管理</span>
					</div>
