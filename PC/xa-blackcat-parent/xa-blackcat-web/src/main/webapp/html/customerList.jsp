...
<html>
        ...
        <script type="text/javascript">
        	...
        	function displayContent(){
	        	...
	        	if(currentModel==null){  
    				...
    				/******************从页面中取出页面中的条件*******************************/
										  ...
										  district=$("#search_district").val();
					    					  ...
					    					  serviceArea=$("#search_serviceArea").val();
					    					  serviceObject=$("#search_serviceObject").val();
        			...
        			/******************从sessionStorge中取出页面中的条件*******************************/
        			  					  name=sessionStorage.getItem(currentModel+".name");
					   					  manager=sessionStorage.getItem(currentModel+".manager");
										  ...
										  district=sessionStorage.getItem(currentModel+".district");
					   					  ...
					   					  serviceArea=sessionStorage.getItem(currentModel+".serviceArea");
					   					  serviceObject=sessionStorage.getItem(currentModel+".serviceObject");
					   					  intro=sessionStorage.getItem(currentModel+".intro");
					   					  file=sessionStorage.getItem(currentModel+".file");
					     				/******************从sessionStorge中取出页面中的条件*******************************/
    				/******************将原来记住的值传重新赋值到页面上*******************************/
    				      				$("#search_name").val(name);
    				        				$("#search_manager").val(manager);
    				    				...
    				    				$("#search_district").val(district);
    				        				...
    				        				$("#search_serviceArea").val(serviceArea);
    				        				$("#search_serviceObject").val(serviceObject);
    				        				$("#search_intro").val(intro);
    				        				$("#search_file").val(file);
    				      				/******************将原来记住的值传重新赋值到页面上*******************************/
				...
				/************************拼装查询条件************************************/
					...
					_jsonFilter += "'search_EQ_manager':'"+manager+"',";
				}
					...
					_jsonFilter += "'search_EQ_serviceArea':'"+serviceArea+"',";
				}
			        					if(serviceObject!=null && serviceObject.length>0){
					_jsonFilter += "'search_EQ_serviceObject':'"+serviceObject+"',";
				}
			        					if(intro!=null && intro.length>0){
					_jsonFilter += "'search_EQ_intro':'"+intro+"',";
				}
			        					if(file!=null && file.length>0){
					_jsonFilter += "'search_EQ_file':'"+file+"',";
				}
			      				/************************拼装查询条件************************************/
				if(_jsonFilter != "{"){
				...
				_jsonFilter += "}";
				var url = "../cms/customer/findCustomerNEStatusPage";
				$.ajax({
					url:url,
					dataType:'json',
					type:'POST',
			...
			$(function(){
				...
				$("#reset").click(function(){
					    				 ...
					    				 $("#search_district").val('');
					        				 $("#search_comment").val('');
					        				 $("#search_serviceArea").val('');
					        				 $("#search_serviceObject").val('');
					        				 $("#search_intro").val('');
					        				 $("#search_file").val('');
					  				});
				...
				$("#updateSome").click(function(){
					...
					rememberRecord();
					var url = "customerEdit.jsp?id="+updateArray[0];
			    	window.location.href=url;
				});
				...
				$("#deleteSome").click(function() {
                    ...
                    if(confirm("你确定要删除选中项吗？")){
                    	var url="../cms/customer/operateCustomerByIds?modelIds=" + delArray.join(",")+"&token="+encodeURIComponent(token);;
                    	$.ajax({
							url: url,
							type: 'post',
							dataType: 'json',
				...
				$("#lockSome").click(function() {
                    ...
                    if(confirm("你确定要锁定选中项吗？")){
                    	var url="../cms/customer/operateCustomerByIds";
                    	$.ajax({
							url: url,
							type: 'post',
							data:{
    ...
    <body>
    <input type="hidden" id="currentModel" value="Customer" />
		<section>
			<div class="col-xs-12">
				<div class="box">
					<div class="span1">
						<span class="badge badge-info">XXXX   &gt; 客户管理</span>
					</div>
