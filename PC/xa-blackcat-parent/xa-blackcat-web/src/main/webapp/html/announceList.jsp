...
<html>
        ...
        <script type="text/javascript">
        	...
        	function displayContent(){
	        	...
	        	if(currentModel==null){  
    				...
    				/******************从页面中取出页面中的条件*******************************/
    				  					  ayear=$("#search_ayear").val();
					    					  yearOrders=$("#search_yearOrders").val();
					    					  byear=$("#search_byear").val();
					    					  bmonth=$("#search_bmonth").val();
					    					  monthOrders=$("#search_monthOrders").val();
					      				/******************从页面中取出页面中的条件*******************************/
        		}else{
        			...
        			/******************从sessionStorge中取出页面中的条件*******************************/
        			  					  ayear=sessionStorage.getItem(currentModel+".ayear");
					   					  yearOrders=sessionStorage.getItem(currentModel+".yearOrders");
					   					  byear=sessionStorage.getItem(currentModel+".byear");
					   					  bmonth=sessionStorage.getItem(currentModel+".bmonth");
					   					  monthOrders=sessionStorage.getItem(currentModel+".monthOrders");
					     				/******************从sessionStorge中取出页面中的条件*******************************/
    				/******************将原来记住的值传重新赋值到页面上*******************************/
    				      				$("#search_ayear").val(ayear);
    				        				$("#search_yearOrders").val(yearOrders);
    				        				$("#search_byear").val(byear);
    				        				$("#search_bmonth").val(bmonth);
    				        				$("#search_monthOrders").val(monthOrders);
    				      				/******************将原来记住的值传重新赋值到页面上*******************************/
				...
				/************************拼装查询条件************************************/
					...
					_jsonFilter += "'search_EQ_ayear':'"+ayear+"',";
				}
			        					if(yearOrders!=null && yearOrders.length>0){
					_jsonFilter += "'search_EQ_yearOrders':'"+yearOrders+"',";
				}
			        					if(byear!=null && byear.length>0){
					_jsonFilter += "'search_EQ_byear':'"+byear+"',";
				}
			        					if(bmonth!=null && bmonth.length>0){
					_jsonFilter += "'search_EQ_bmonth':'"+bmonth+"',";
				}
			        					if(monthOrders!=null && monthOrders.length>0){
					_jsonFilter += "'search_EQ_monthOrders':'"+monthOrders+"',";
				}
			      				/************************拼装查询条件************************************/
				if(_jsonFilter != "{"){
				...
				_jsonFilter += "}";
				var url = "../cms/announce/findAnnounceNEStatusPage";
				$.ajax({
					url:url,
					dataType:'json',
					type:'POST',
			...
			$(function(){
				...
				$("#reset").click(function(){
					      				 $("#search_ayear").val('');
					        				 $("#search_yearOrders").val('');
					        				 $("#search_byear").val('');
					        				 $("#search_bmonth").val('');
					        				 $("#search_monthOrders").val('');
					  				});
						
				//点击新增按钮
				$("#addSome").click(function(){
					rememberRecord();
					var url = "announceEdit.jsp";
				...
				$("#updateSome").click(function(){
					...
					rememberRecord();
					var url = "announceEdit.jsp?id="+updateArray[0];
			    	window.location.href=url;
				});
				...
				$("#deleteSome").click(function() {
                    ...
                    if(confirm("你确定要删除选中项吗？")){
                    	var url="../cms/announce/operateAnnounceByIds?modelIds=" + delArray.join(",")+"&token="+encodeURIComponent(token);;
                    	$.ajax({
							url: url,
							type: 'post',
							dataType: 'json',
				...
				$("#lockSome").click(function() {
                    ...
                    if(confirm("你确定要锁定选中项吗？")){
                    	var url="../cms/announce/operateAnnounceByIds";
                    	$.ajax({
							url: url,
							type: 'post',
							data:{
    ...
    <body>
    <input type="hidden" id="currentModel" value="Announce" />
		<section>
			<div class="col-xs-12">
				<div class="box">
					<div class="span1">
						<span class="badge badge-info">XXXX   &gt; 公告管理</span>
					</div>
