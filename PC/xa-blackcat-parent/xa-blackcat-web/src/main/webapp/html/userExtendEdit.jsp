    ...
    <head>
        ...
        <script type="text/javascript">
			...
			$(function(){
				...
				$("#tid").val(tid);
				var url ='../cms/userExtend/findUserExtendById';
				...
				$("#demo_31").validator({
					valid:function(form){
						...
						if(typeof(tid)=="undefined" || tid == "" || tid == "0"){
							//新增
							tid = 0;
							submit_url = "../cms/userExtend/saveUserExtend";
						}
						else{
							submit_url = "../cms/userExtend/updateUserExtend";
						}
			        	/** 富文本操作预留,如需要可打开此注释
						...
						$.ajax({
							...
							success:function(data){
								if(data.code==1){
									window.location.href="userExtendList.jsp";
								}
								else{
									alert(data.message);
								}
				...
				$(document).on('change','#uploadPhotoFile',function(){
					$.ajaxFileUpload({
						url:'../cms/userExtend/photoUpload',
						secureuri:false,
    ...
    <body>
    	<section>
      	<form id="demo_31" class="form-inline">
        	<div class="col-xs-12">
				<div class="span1">
					<span class="badge badge-info">XX管理    &gt; 用户扩展管理    &gt;  用户扩展编辑</span>
				</div>
              	<div class="box">
					<div class="box-header">
						<br>
							<input type="button" value="确定" id="saveOrupdate" class="btn btn-success">
							<a href="#" onclick="document.location.href='userExtendList.jsp';" class="btn btn-info" >返回</a>
					</div>
					<div class="tab" id="usercontent">
						<input type="hidden" id="tid"/>
						<fieldset>
						  								 								<li class="controls">用户名称:</li>
								<li style="width:300px;text-align:left;">
								<input type="text" placeholder="请填写用户名称" data-rule="用户名称:required;length[0~50]" id="name" name="name" value="" />
								</li>
								  								<li class="controls">用户id:</li>
								<li style="width:300px;text-align:left;">
								<input type="text" placeholder="请填写用户id" data-rule="用户id:required;length[0~10]" id="userId" name="userId" value="" />
								</li>
								 						</fieldset>
						<br/>
					</div>
