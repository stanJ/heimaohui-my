				...
				$("#demo_31").validator({
					valid:function(form){
						...
						if(typeof(tid)=="undefined" || tid == "" || tid == "0"){
							//新增
							tid = 0;
							submit_url = "../cms/pics/savePics";
						}
						else{
							submit_url = "../cms/pics/updatePics";
						}
			        	/** 富文本操作预留,如需要可打开此注释
						...
						$.ajax({
							...
							success:function(data){
								if(data.code==1){
									window.location.href="picsList.jsp";
								}
								else{
									alert(data.message);
								}
    ...
    <body>
    	<section>
      	<form id="demo_31" class="form-inline">
        	<div class="col-xs-12">
				<div class="span1">
					<span class="badge badge-info">XX管理    &gt; 宣传照片管理    &gt;  宣传照片编辑</span>
				</div>
              	<div class="box">
					<div class="box-header">
						<br>
							<input type="button" value="确定" id="saveOrupdate" class="btn btn-success">
							<a href="#" onclick="document.location.href='picsList.jsp';" class="btn btn-info" >返回</a>
					</div>
					<div class="tab" id="usercontent">
						...
						<fieldset>
								...
								<input type="text" placeholder="请填写序号" data-rule="序号:required;length[0~10]" id="serial" name="serial" value="" />
								  								...
								  								<li class="controls">照片:</li>
								<li style="width:300px;text-align:left;">
								<input type="text" placeholder="请填写照片" data-rule="照片:required;length[0~200]" id="pic" name="pic" value="" />
								</li>
								 						</fieldset>
						<br/>
					</div>
