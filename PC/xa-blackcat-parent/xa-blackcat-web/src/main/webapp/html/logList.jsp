...
<html>
        ...
        <script type="text/javascript">
        	...
        	function displayContent(){
	        	...
	        	if(currentModel==null){  
    				...
    				/******************从页面中取出页面中的条件*******************************/
    				  					  client=$("#search_client").val();
					    					  ...
					    					  ldate=$("#search_ldate").val();
					    					  service=$("#search_service").val();
					    					  method=$("#search_method").val();
					    					  usedTimes=$("#search_usedTimes").val();
        			...
        			/******************从sessionStorge中取出页面中的条件*******************************/
        			  					  client=sessionStorage.getItem(currentModel+".client");
					   					  userId=sessionStorage.getItem(currentModel+".userId");
					   					  ldate=sessionStorage.getItem(currentModel+".ldate");
					   					  service=sessionStorage.getItem(currentModel+".service");
					   					  method=sessionStorage.getItem(currentModel+".method");
					   					  usedTimes=sessionStorage.getItem(currentModel+".usedTimes");
					   					  result=sessionStorage.getItem(currentModel+".result");
					     				/******************从sessionStorge中取出页面中的条件*******************************/
    				/******************将原来记住的值传重新赋值到页面上*******************************/
    				      				$("#search_client").val(client);
    				        				$("#search_userId").val(userId);
    				        				$("#search_ldate").val(ldate);
    				        				$("#search_service").val(service);
    				        				$("#search_method").val(method);
    				        				$("#search_usedTimes").val(usedTimes);
				...
				/************************拼装查询条件************************************/
					...
					_jsonFilter += "'search_EQ_client':'"+client+"',";
				}
			        					if(userId!=null && userId.length>0){
					_jsonFilter += "'search_EQ_userId':'"+userId+"',";
				}
			        					if(ldate!=null && ldate.length>0){
					_jsonFilter += "'search_EQ_ldate':'"+ldate+"',";
				}
			        					if(service!=null && service.length>0){
					_jsonFilter += "'search_EQ_service':'"+service+"',";
				}
			        					if(method!=null && method.length>0){
					_jsonFilter += "'search_EQ_method':'"+method+"',";
				}
			        					if(usedTimes!=null && usedTimes.length>0){
					_jsonFilter += "'search_EQ_usedTimes':'"+usedTimes+"',";
				}
			        					if(result!=null && result.length>0){
					_jsonFilter += "'search_EQ_result':'"+result+"',";
				}
			      				/************************拼装查询条件************************************/
				if(_jsonFilter != "{"){
				...
				_jsonFilter += "}";
				var url = "../cms/log/findLogNEStatusPage";
				$.ajax({
					url:url,
					dataType:'json',
					type:'POST',
			...
			$(function(){
				...
				$("#updateSome").click(function(){
					...
					rememberRecord();
					var url = "logEdit.jsp?id="+updateArray[0];
			    	window.location.href=url;
				});
				...
				$("#deleteSome").click(function() {
                    ...
                    if(confirm("你确定要删除选中项吗？")){
                    	var url="../cms/log/operateLogByIds?modelIds=" + delArray.join(",");
                    	$.ajax({
							url: url,
							type: 'post',
							dataType: 'json',
				...
				$("#lockSome").click(function() {
                    ...
                    if(confirm("你确定要锁定选中项吗？")){
                    	var url="../cms/log/operateLogByIds";
                    	$.ajax({
							url: url,
							type: 'post',
							data:{
    ...
    <body>
    <input type="hidden" id="currentModel" value="Log" />
		<section>
			<div class="col-xs-12">
				<div class="box">
					<div class="span1">
						<span class="badge badge-info">XXXX   &gt; 日志管理</span>
					</div>
