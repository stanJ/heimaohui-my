    ...
    <head>
        ...
        <script type="text/javascript">
			...
			$(function(){
				...
				$("#tid").val(tid);
				var url ='../cms/doctor/findDoctorById';
				if(tid!=null && tid!=0 && tid!=""){
					$.ajax({
						...
						success:function(data){
							if(data.code == 1){
								/********************加载数据*****************************/
								  									$("#hospitalId").val(data.object.hospitalId);
								     									$("#name").val(data.object.name);
								     									$("#gender").val(data.object.gender);
								     									$("#age").val(data.object.age);
								     									$("#titile").val(data.object.titile);
								     									$("#degree").val(data.object.degree);
								     									$("#speciality").val(data.object.speciality);
								     									$("#userExtendId").val(data.object.userExtendId);
								   								/********************加载数据*****************************/
								/********************加载字典数据*****************************/
				...
				$("#demo_31").validator({
					valid:function(form){
						...
						if(typeof(tid)=="undefined" || tid == "" || tid == "0"){
							//新增
							tid = 0;
							submit_url = "../cms/doctor/saveDoctor";
						}
						else{
							submit_url = "../cms/doctor/updateDoctor";
						}
			        	/** 富文本操作预留,如需要可打开此注释
						...
						$.ajax({
							...
							success:function(data){
								if(data.code==1){
									window.location.href="doctorList.jsp";
								}
								else{
									alert(data.message);
								}
    ...
    <body>
    	<section>
      	<form id="demo_31" class="form-inline">
        	<div class="col-xs-12">
				<div class="span1">
					<span class="badge badge-info">XX管理    &gt; 医生管理    &gt;  医生编辑</span>
				</div>
              	<div class="box">
					<div class="box-header">
						<br>
							<input type="button" value="确定" id="saveOrupdate" class="btn btn-success">
							<a href="#" onclick="document.location.href='doctorList.jsp';" class="btn btn-info" >返回</a>
					</div>
					<div class="tab" id="usercontent">
						<input type="hidden" id="tid"/>
						<fieldset>
						        								    <depend:list  key="Hospital.id" show="Hospital.name" id="hospitalId" name="hospitalId" title="医院" titletag="li" titletagclass="controls" outtag="li"
								...
								<input type="text" placeholder="请填写姓名" data-rule="姓名:required;length[0~50]" id="name" name="name" value="" />
									...
									outtagstyle="width: 300px; text-align: left;" datarule="required;"/>
									 								  								<li class="controls">年龄:</li>
								<li style="width:300px;text-align:left;">
								<input type="text" placeholder="请填写年龄" data-rule="length[0~10]" id="age" name="age" value="" />
								</li>
								  								<li class="controls">职称:</li>
								<li style="width:300px;text-align:left;">
								<input type="text" placeholder="请填写职称" data-rule="length[0~10]" id="titile" name="titile" value="" />
								</li>
								  								<li class="controls">学历:</li>
								<li style="width:300px;text-align:left;">
								<input type="text" placeholder="请填写学历" data-rule="length[0~10]" id="degree" name="degree" value="" />
								</li>
								  								<li class="controls">专长:</li>
								<li style="width:300px;text-align:left;">
								<input type="text" placeholder="请填写专长" data-rule="length[0~500]" id="speciality" name="speciality" value="" />
								</li>
								     <depend:list  key="UserExtend.id" show="UserExtend.name" id="userExtendId" name="userExtendId" title=" 用户扩展id" titletag="li" titletagclass="controls" outtag="li"
									outtagstyle="width: 300px; text-align: left;" datarule="required;"/>
								   								 						</fieldset>
						<br/>
					</div>
