    ...
    <head>
        ...
        <script type="text/javascript">
			...
			$(function(){
				...
				$("#tid").val(tid);
				var url ='../cms/hospital/findHospitalById';
						...
						success:function(data){
							if(data.code == 1){
								/********************加载数据*****************************/
								   								...
								   								/********************加载数据*****************************/
								/********************加载字典数据*****************************/
								    								setDict_DISTRICT(data.object.province+","+data.object.city+","+data.object.district );     								/********************加载字典数据*****************************/
							}else{
								alert(data.message);
							}
						}
				...
				$("#demo_31").validator({
					valid:function(form){
						...
						if(typeof(tid)=="undefined" || tid == "" || tid == "0"){
							//新增
							tid = 0;
							submit_url = "../cms/hospital/saveHospital";
						}
						else{
							submit_url = "../cms/hospital/updateHospital";
						}
			        	/** 富文本操作预留,如需要可打开此注释
						...
						$.ajax({
							...
							success:function(data){
								if(data.code==1){
									window.location.href="hospitalList.jsp";
								}
								else{
									alert(data.message);
								}
    ...
    <body>
    	<section>
      	<form id="demo_31" class="form-inline">
        	<div class="col-xs-12">
				<div class="span1">
					<span class="badge badge-info">XX管理    &gt; 医院管理    &gt;  医院编辑</span>
				</div>
              	<div class="box">
					<div class="box-header">
						<br>
							<input type="button" value="确定" id="saveOrupdate" class="btn btn-success">
							<a href="#" onclick="document.location.href='hospitalList.jsp';" class="btn btn-info" >返回</a>
					</div>
					<div class="tab" id="usercontent">
						...
						<fieldset>
								...
								<input type="text" placeholder="请填写名称" data-rule="名称:required;length[0~500]" id="name" name="name" value="" />
								</li>
								   								<dict:list  key="DISTRICT" id="province,city,district" name="province,city,district" title="省,市,区" titletag="li" titletagclass="controls" outtag="li"
									outtagstyle="width: 300px; text-align: left;" datarule="required;"/>
									 								  								<li class="controls">地址:</li>
								<li style="width:300px;text-align:left;">
								<input type="text" placeholder="请填写地址" data-rule="length[0~500]" id="address" name="address" value="" />
								</li>
								 						</fieldset>
						<br/>
					</div>
