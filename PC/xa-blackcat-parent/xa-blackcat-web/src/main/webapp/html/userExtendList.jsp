...
<html>
    <head>
        ...
        <script type="text/javascript">
        	...
        	function displayContent(){
				...
				_jsonFilter += "}";
				var url = "../cms/userExtend/findUserExtendNEStatusPage";
				$.ajax({
					url:url,
					dataType:'json',
					type:'POST',
			...
			$(function(){
				...
				$("#updateSome").click(function(){
					...
					rememberRecord();
					var url = "userExtendEdit.jsp?id="+updateArray[0];
			    	window.location.href=url;
				});
				...
				$("#deleteSome").click(function() {
                    ...
                    if(confirm("你确定要删除选中项吗？")){
                    	var url="../cms/userExtend/operateUserExtendByIds?modelIds=" + delArray.join(",")+"&token="+encodeURIComponent(token);;
                    	$.ajax({
							url: url,
							type: 'post',
							dataType: 'json',
				...
				$("#lockSome").click(function() {
                    ...
                    if(confirm("你确定要锁定选中项吗？")){
                    	var url="../cms/userExtend/operateUserExtendByIds";
                    	$.ajax({
							url: url,
							type: 'post',
							data:{
    ...
    <body>
    <input type="hidden" id="currentModel" value="UserExtend" />
		<section>
			<div class="col-xs-12">
				<div class="box">
					<div class="span1">
						<span class="badge badge-info">XXXX   &gt; 用户扩展管理</span>
					</div>
