...
<html>
        ...
        <script type="text/javascript">
        	...
        	function rememberRecord(){
				...
				/*****************记录当前的条件**********************************/
				  					sessionStorage.setItem(currentModel+".name",$("#search_name").val());
				    					sessionStorage.setItem(currentModel+".province",$("#search_province").val());
									sessionStorage.setItem(currentModel+".city",$("#search_city").val());
									sessionStorage.setItem(currentModel+".district",$("#search_district").val());
				    					sessionStorage.setItem(currentModel+".address",$("#search_address").val());
				  				/*****************记录当前的条件**********************************/
        	...
        	function displayContent(){
	        	...
	        	if(currentModel==null){  
        			...
        			/******************从sessionStorge中取出页面中的条件*******************************/
										  ...
										  district=sessionStorage.getItem(currentModel+".district");
					   					  address=sessionStorage.getItem(currentModel+".address");
					     				/******************从sessionStorge中取出页面中的条件*******************************/
    				/******************将原来记住的值传重新赋值到页面上*******************************/
				...
				/************************拼装查询条件************************************/
					...
					_jsonFilter += "'search_LIKE_name':'"+name+"',";
				}
					...
					_jsonFilter += "'search_LIKE_address':'"+address+"',";
				}
			      				/************************拼装查询条件************************************/
				if(_jsonFilter != "{"){
				...
				_jsonFilter += "}";
				var url = "../cms/hospital/findHospitalNEStatusPage";
				$.ajax({
					url:url,
					dataType:'json',
					type:'POST',
			...
			$(function(){
				...
				$("#updateSome").click(function(){
					...
					rememberRecord();
					var url = "hospitalEdit.jsp?id="+updateArray[0];
			    	window.location.href=url;
				});
				...
				$("#deleteSome").click(function() {
                    ...
                    if(confirm("你确定要删除选中项吗？")){
                    	var url="../cms/hospital/operateHospitalByIds?modelIds=" + delArray.join(",")+"&token="+encodeURIComponent(token);;
                    	$.ajax({
							url: url,
							type: 'post',
							dataType: 'json',
				...
				$("#lockSome").click(function() {
                    ...
                    if(confirm("你确定要锁定选中项吗？")){
                    	var url="../cms/hospital/operateHospitalByIds";
                    	$.ajax({
							url: url,
							type: 'post',
							data:{
    ...
    <body>
    <input type="hidden" id="currentModel" value="Hospital" />
		<section>
			<div class="col-xs-12">
				<div class="box">
					<div class="span1">
						<span class="badge badge-info">XXXX   &gt; 医院管理</span>
					</div>
					<div class="box-header">
						<form id="search_form" class="form-inline" >
							<div class="tabSelect">
																 <li><span class="add-on">名称:</span></li><li><input type="text" name="name" placeholder="请输入名称" id="search_name"/></li>
																 <dict:list  key="DISTRICT" id="province,city,district" idprefix="search_"  name="province,city,district" title="省,市,区" titletag="li span"
								titletagclass="add-on" outtag="li"/>
								 																 <li><span class="add-on">地址:</span></li><li><input type="text" name="address" placeholder="请输入地址" id="search_address"/></li>
															</div>
							<br />  
						</form>
					</div>
						...
						<script id="tableContentTmple" type="text/x-jsrender">
							<tr>
								<td><input id="ck_{{:tid}}" ckId="{{:tid}}" ckStatus="{{:status}}" class="ckSelect" type="checkbox" /></td>
								 								   								  <td>{{:name}}</td> 
								   								  								   								  <td>{{:converted_province}}</td> 
								   																   								  <td>{{:converted_city}}</td> 
								   																   								  <td>{{:converted_district}}</td> 
								   								  								   								  <td>{{:address}}</td> 
