...
<html>
    <head>
        ...
        <script type="text/javascript">
        	...
        	function displayContent(){
				...
				_jsonFilter += "}";
				var url = "../cms/assistant/findAssistantNEStatusPage";
				$.ajax({
					url:url,
					dataType:'json',
					type:'POST',
			...
			$(function(){
				...
				$("#updateSome").click(function(){
					...
					rememberRecord();
					var url = "assistantEdit.jsp?id="+updateArray[0];
			    	window.location.href=url;
				});
				...
				$("#deleteSome").click(function() {
                    ...
                    if(confirm("你确定要删除选中项吗？")){
                    	var url="../cms/assistant/operateAssistantByIds?modelIds=" + delArray.join(",")+"&token="+encodeURIComponent(token);;
                    	$.ajax({
							url: url,
							type: 'post',
							dataType: 'json',
				...
				$("#lockSome").click(function() {
                    ...
                    if(confirm("你确定要锁定选中项吗？")){
                    	var url="../cms/assistant/operateAssistantByIds";
                    	$.ajax({
							url: url,
							type: 'post',
							data:{
    ...
    <body>
    <input type="hidden" id="currentModel" value="Assistant" />
		<section>
			<div class="col-xs-12">
				<div class="box">
					<div class="span1">
						<span class="badge badge-info">XXXX   &gt; 助手信息管理</span>
					</div>
