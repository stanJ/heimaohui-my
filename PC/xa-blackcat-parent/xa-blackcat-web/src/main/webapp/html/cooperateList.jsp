...
<html>
        ...
        <script type="text/javascript">
        	...
        	function displayContent(){
	        	...
	        	if(currentModel==null){  
    				...
    				/******************从页面中取出页面中的条件*******************************/
    				  					  expertId=$("#search_expertId").val();
					    					  ...
					    					  actionDate=$("#search_actionDate").val();
					    					  actionAddress=$("#search_actionAddress").val();
					    					  contractPrice=$("#search_contractPrice").val();
					    					  costPrice=$("#search_costPrice").val();
					    					  expertContactor=$("#search_expertContactor").val();
					    					  comment=$("#search_comment").val();
					    					  pics=$("#search_pics").val();
					      				/******************从页面中取出页面中的条件*******************************/
        		}else{
        			...
        			/******************从sessionStorge中取出页面中的条件*******************************/
        			  					  expertId=sessionStorage.getItem(currentModel+".expertId");
					   					  customerName=sessionStorage.getItem(currentModel+".customerName");
					   					  actionDate=sessionStorage.getItem(currentModel+".actionDate");
					   					  actionAddress=sessionStorage.getItem(currentModel+".actionAddress");
					   					  contractPrice=sessionStorage.getItem(currentModel+".contractPrice");
					   					  costPrice=sessionStorage.getItem(currentModel+".costPrice");
					   					  expertContactor=sessionStorage.getItem(currentModel+".expertContactor");
					   					  comment=sessionStorage.getItem(currentModel+".comment");
					   					  pics=sessionStorage.getItem(currentModel+".pics");
					     				/******************从sessionStorge中取出页面中的条件*******************************/
    				/******************将原来记住的值传重新赋值到页面上*******************************/
    				      				$("#search_expertId").val(expertId);
    				        				$("#search_customerName").val(customerName);
    				        				$("#search_actionDate").val(actionDate);
    				        				$("#search_actionAddress").val(actionAddress);
    				        				$("#search_contractPrice").val(contractPrice);
    				        				$("#search_costPrice").val(costPrice);
    				        				$("#search_expertContactor").val(expertContactor);
    				        				$("#search_comment").val(comment);
    				        				$("#search_pics").val(pics);
    				      				/******************将原来记住的值传重新赋值到页面上*******************************/
				...
				/************************拼装查询条件************************************/
					...
					_jsonFilter += "'search_EQ_customerName':'"+customerName+"',";
				}
			        					if(actionDate!=null && actionDate.length>0){
					_jsonFilter += "'search_EQ_actionDate':'"+actionDate+"',";
				}
			        					if(actionAddress!=null && actionAddress.length>0){
					_jsonFilter += "'search_EQ_actionAddress':'"+actionAddress+"',";
				}
			        					if(contractPrice!=null && contractPrice.length>0){
					_jsonFilter += "'search_EQ_contractPrice':'"+contractPrice+"',";
				}
			        					if(costPrice!=null && costPrice.length>0){
					_jsonFilter += "'search_EQ_costPrice':'"+costPrice+"',";
				}
			        					if(expertContactor!=null && expertContactor.length>0){
					_jsonFilter += "'search_EQ_expertContactor':'"+expertContactor+"',";
				}
			        					if(comment!=null && comment.length>0){
					_jsonFilter += "'search_EQ_comment':'"+comment+"',";
				}
			        					if(pics!=null && pics.length>0){
					_jsonFilter += "'search_EQ_pics':'"+pics+"',";
				}
			      				/************************拼装查询条件************************************/
				if(_jsonFilter != "{"){
				...
				_jsonFilter += "}";
				var url = "../cms/cooperate/findCooperateNEStatusPage";
				$.ajax({
					url:url,
					dataType:'json',
					type:'POST',
			...
			$(function(){
				...
				$("#reset").click(function(){
					      				 $("#search_expertId").val('');
					        				 ...
					        				 $("#search_actionDate").val('');
					        				 $("#search_actionAddress").val('');
					        				 $("#search_contractPrice").val('');
					        				 $("#search_costPrice").val('');
					        				 $("#search_expertContactor").val('');
					        				 $("#search_comment").val('');
					        				 $("#search_pics").val('');
					  				});
				...
				$("#updateSome").click(function(){
					...
					rememberRecord();
					var url = "cooperateEdit.jsp?id="+updateArray[0];
			    	window.location.href=url;
				});
				...
				$("#deleteSome").click(function() {
                    ...
                    if(confirm("你确定要删除选中项吗？")){
                    	var url="../cms/cooperate/operateCooperateByIds?modelIds=" + delArray.join(",")+"&token="+encodeURIComponent(token);;
                    	$.ajax({
							url: url,
							type: 'post',
							dataType: 'json',
				...
				$("#lockSome").click(function() {
                    ...
                    if(confirm("你确定要锁定选中项吗？")){
                    	var url="../cms/cooperate/operateCooperateByIds";
                    	$.ajax({
							url: url,
							type: 'post',
							data:{
    ...
    <body>
    <input type="hidden" id="currentModel" value="Cooperate" />
		<section>
			<div class="col-xs-12">
				<div class="box">
					<div class="span1">
						<span class="badge badge-info">XXXX   &gt; 配合度管理</span>
					</div>
