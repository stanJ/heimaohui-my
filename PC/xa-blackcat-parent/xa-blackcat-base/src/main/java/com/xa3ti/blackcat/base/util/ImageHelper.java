...
public class ImageHelper {  
     ...
     * 图片等比缩放 
     */  
    public static void zoomImageSynRate(String src,String dest,int w,int h) throws Exception {  
        File srcFile = new File(src);  
        BufferedImage bufImg = ImageIO.read(srcFile);  
        Integer height = bufImg.getHeight();
        Integer width = bufImg.getWidth();
        if(height / width > h / w){
        	w = h * width / height;
        }
        else{
        ...
        ImageHelper.zoomImage(src, dest, w, h);
    }  
    
    public static void main(String[] args) throws Exception {  
        ImageHelper.zoomImageSynRate("C:\\temp\\mainpic2.png", "C:\\temp\\mainpic21.png", 800, 600);
    }  
}  
