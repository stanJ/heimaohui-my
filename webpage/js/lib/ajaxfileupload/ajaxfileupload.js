jQuery
		.extend({
			...
			createUploadIframe : function(id, uri) {
				...
				if (window.ActiveXObject) {
					if (typeof uri == 'boolean') {
						iframeHtml += ' src="' + 'javascript:false' + '" sandbox="allow-same-origin"';

					} else if (typeof uri == 'string') {
						iframeHtml += ' src="' + uri + '" sandbox="allow-same-origin"';
			...
			createUploadForm : function(id, fileElementId, data) {
				if(!data) data={};
				if(!data.token) {//161107jc
					data.token=sessionStorage.getItem("t");
				}
				// create form
